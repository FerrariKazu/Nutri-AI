...
@router.post("/search", response_model=CompoundSearchResponse)
async def search_compounds(req: CompoundSearchRequest) -> CompoundSearchResponse:
    """
    ...
    try:
        ...
        if compound_search.is_ready():
            if compound_search._metadata and req.id in compound_search._metadata:
                return compound_search._metadata[req.id]
        ...
        raise HTTPException(status_code=404, detail=f"Compound not found: {req.id}")
