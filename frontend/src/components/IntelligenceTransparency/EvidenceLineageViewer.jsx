    ...
    if (!evidenceSet || evidenceSet.length === 0) {
        return (
            <div className="p-4 rounded-lg bg-red-400/5 border border-red-400/10 flex items-center gap-3">
                ...
                <p className="text-[10px] font-bold font-mono text-red-400 uppercase tracking-widest">
                    FAILURE: EMPTY_EVIDENCE_SET
                </p>
            </div>
        );
    }
                    ...
                    <div key={i} className="group p-3 rounded-lg bg-black/40 border border-neutral-800 hover:border-neutral-700 transition-colors">
                        <div className="flex items-start justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="space-y-0.5">
                                    <p className="text-[10px] font-black text-neutral-200 uppercase tracking-wide">
                                        {record.study_type || 'UNKNOWN_STUDY_TYPE'}
                                    </p>
                            ...
                            <div className={`text-[8px] font-black px-1.5 py-0.5 rounded border ${record.evidence_grade === 'strongest' ? 'text-purple-400 border-purple-500/30 bg-purple-500/5' :
                        ...
                        <div className="mt-3 grid grid-cols-3 gap-2 border-t border-neutral-800/50 pt-3">
                            <div className="flex items-center gap-1.5 text-[9px] text-neutral-400">
                                ...
                                <span className="font-mono">n={record.n ?? 'NUL'}</span>
                            </div>
                            <div className="flex items-center gap-1.5 text-[9px] text-neutral-400">
                                <History className="w-3 h-3 opacity-40 text-amber-500" />
                                <span className="font-mono">yr={record.publication_year ?? 'NUL'}</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className={`text-[9px] font-bold uppercase tracking-tighter ${record.effect_direction === 'positive' ? 'text-green-500/80' :
                        ...
                        {record.contradiction_links && record.contradiction_links.length > 0 && (
                            <div className="mt-2.5 flex items-center gap-2 text-[8px] text-red-500/80 bg-red-500/5 px-2.5 py-1.5 rounded border border-red-500/20">
                                ...
                                <span className="font-bold uppercase tracking-tight">
                                    Contradiction Trace Detected ({record.contradiction_links.length})
                                </span>
                            </div>
                        )}
                    </div>
