...
import { ShieldCheck, AlertTriangle, Info, CheckCircle2, FlaskConical, HelpCircle } from 'lucide-react';
...
export const TierBadge = ({ tier, label }) => {
    ...
    return (
        <div className={`px-2 py-0.5 rounded border text-[9px] font-bold uppercase tracking-tighter ${colors[tier]}`}>
            {label || `Tier ${tier}`}
        </div>
    );
};
    ...
    return (
        <div className="space-y-1.5 w-full">
            <div className="flex justify-between items-end">
                <span className="text-[10px] text-neutral-500 font-mono uppercase tracking-widest">{label}</span>
                <span className="text-[10px] font-bold text-neutral-300 font-mono">{percentage}%</span>
            </div>
...
export const RiskThermometer = ({ severity }) => {
    ...
    const color = severity > 0.7 ? 'text-red-400' : severity > 0.3 ? 'text-amber-400' : 'text-green-400';

    return (
        <div className="flex items-center gap-3">
            <div className="shrink-0 h-14 w-2 rounded-full bg-neutral-800 relative overflow-hidden">
                <div
                    className={`absolute bottom-0 w-full transition-all duration-1000 ${severity > 0.7 ? 'bg-red-500' : severity > 0.3 ? 'bg-amber-500' : 'bg-green-500'}`}
                    style={{ height: `${percentage}%` }}
                />
            </div>
            <div className="space-y-0.5">
                <div className="flex items-center gap-1.5">
                    <p className="text-[10px] text-neutral-500 font-mono uppercase tracking-widest">Risk Severity</p>
                ...
                <p className={`text-xs font-bold ${color}`}>
                    {severity > 0.7 ? 'High / Clinical' : severity > 0.3 ? 'Moderate / Caution' : 'Low / Minimal'}
                </p>
            </div>
        </div>
    );
...
export const ResponsibleDecision = ({ meta }) => {
    ...
    return (
        <div className={`p-4 rounded-lg border flex flex-col items-center text-center gap-2 ${toneColors[tone]} transition-all duration-500 hover:shadow-lg`}>
            <p className="text-[10px] font-mono uppercase tracking-[0.2em] opacity-60">Recommendation Gate</p>
